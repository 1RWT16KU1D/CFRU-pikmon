#pragma once

enum
{
	CLASS_PKMN_TRAINER_1,	//0x0
	CLASS_PKMN_TRAINER_2,	//0x1
	CLASS_AQUA_LEADER,		//0x2
	CLASS_TEAM_AQUA,		//0x3
	CLASS_AROMA_LADY_RS,	//0x4
	CLASS_RUIN_MANIAC_RS,	//0x5
	CLASS_INTERVIEWER,		//0x6
	CLASS_TUBER_RS_1,		//0x7
	CLASS_TUBER_RS_2,		//0x8
	CLASS_COOLTRAINER_RS,	//0x9
	CLASS_HEX_MANIAC,		//0xA
	CLASS_LADY_RS,			//0xB
	CLASS_BEAUTY_RS,		//0xC
	CLASS_RICH_BOY,			//0xD
	CLASS_POKEMANIAC_RS,	//0xE
	CLASS_PKMN_TRAINER_RSE,	//0xF
	CLASS_BLACK_BELT_RS,	//0x10
	CLASS_GUITARIST_RS,		//0x11
	CLASS_KINDLER,			//0x12
	CLASS_CAMPER_RS,		//0x13
	CLASS_BUG_MANIAC,		//0x14
	CLASS_PSYCHIC_RS,		//0x15
	CLASS_GENTLEMAN_RS,		//0x16
	CLASS_ELITE_FOUR_RS,	//0x17
	CLASS_LEADER_RS,		//0x18
	CLASS_SCHOOL_KID,		//0x19
	CLASS_SR_AND_JR,		//0x1A
	CLASS_POKEFAN,			//0x1B
	CLASS_EXPERT,			//0x1C
	CLASS_YOUNGSTER_RS,		//0x1D
	CLASS_CHAMPION_RS,		//0x1E
	CLASS_FISHERMAN_RS,		//0x1F
	CLASS_TRIATHLETE,		//0x20
	CLASS_DRAGON_TAMER,		//0x21
	CLASS_BIRD_KEEPER_RS,	//0x22
	CLASS_NINJA_BOY,		//0x23
	CLASS_BATTLE_GIRL,		//0x24
	CLASS_PARASOL_LADY,		//0x25
	CLASS_SWIMMER_F_RS,		//0x26
	CLASS_PICNICKER_RS,		//0x27
	CLASS_TWINS_RS,			//0x28
	CLASS_SAILOR_RS,		//0x29
	CLASS_BOARDER,			//0x2A
	CLASS_COLLECTOR,		//0x2B
	CLASS_PKMN_TRAINER_3,	//0x2C
	CLASS_PKMN_BREEDER_RS,	//0x2D
	CLASS_PKMN_RANGER_RS,	//0x2E
	CLASS_MAGMA_LEADER,		//0x2F
	CLASS_TEAM_MAGMA,		//0x30
	CLASS_LASS_RS,			//0x31
	CLASS_BUG_CATCHER_RS,	//0x32
	CLASS_HIKER_RS,			//0x33
	CLASS_YOUNG_COUPLE_RS,	//0x34
	CLASS_OLD_COUPLE,		//0x35
	CLASS_SIS_AND_BRO_RS,	//0x36
	CLASS_AQUA_ADMIN,		//0x37
	CLASS_MAGMA_ADMIN,		//0x38
	CLASS_YOUNGSTER, 		//0x39
    CLASS_BUG_CATCHER, 		//0x3A
    CLASS_LASS, 			//0x3B
    CLASS_SAILOR, 			//0x3C
    CLASS_CAMPER, 			//0x3D
    CLASS_PICNICKER, 		//0x3E
    CLASS_POKEMANIAC, 		//0x3F
    CLASS_SUPER_NERD, 		//0x40
    CLASS_HIKER, 			//0x41
    CLASS_BIKER, 			//0x42
	CLASS_BURGLAR, 			//0x43
	CLASS_ENGINEER, 		//0x44
	CLASS_FISHERMAN, 		//0x45
	CLASS_SWIMMER_M, 		//0x46
	CLASS_CUE_BALL, 		//0x47
	CLASS_GAMER, 			//0x48
	CLASS_BEAUTY, 			//0x49
	CLASS_SWIMMER_F, 		//0x4A
	CLASS_PSYCHIC, 			//0x4B
	CLASS_ROCKER, 			//0x4C
	CLASS_JUGGLER, 			//0x4D
	CLASS_TAMER, 			//0x4E
	CLASS_BIRD_KEEPER,		//0x4F
	CLASS_BLACK_BELT,        // 0x50
	CLASS_DUMMY,             // 0x51
	CLASS_CAPTAIN,           // 0x52
	CLASS_SCIENTIST,         // 0x53
	CLASS_BOSS,              // 0x54
	CLASS_LEADER,            // 0x55
	CLASS_TEAM_ROCKET,       // 0x56
	CLASS_COOLTRAINER,       // 0x57
	CLASS_ELITE_FOUR,        // 0x58
	CLASS_DUMMY_2,           // 0x59
	CLASS_DUMMY_3,           // 0x5A
	CLASS_GENTLEMAN,         // 0x5B
	CLASS_RIVAL_2,           // 0x5C
	CLASS_CHAMPION,          // 0x5D
	CLASS_CHANNELER,         // 0x5E
	CLASS_TWINS,             // 0x5F
	CLASS_COOL_COUPLE,       // 0x60
	CLASS_YOUNG_COUPLE,      // 0x61
	CLASS_CRUSH_KIN,         // 0x62
	CLASS_SIS_AND_BRO,       // 0x63
	CLASS_PKMN_PROF,         // 0x64
	CLASS_PLAYER,            // 0x65
	CLASS_CRUSH_GIRL,        // 0x66
	CLASS_TUBER,             // 0x67
	CLASS_PKMN_BREEDER,      // 0x68
	CLASS_PKMN_RANGER,       // 0x69
	CLASS_AROMA_LADY,        // 0x6A
	CLASS_RUIN_MANIAC,       // 0x6B
	CLASS_LADY,              // 0x6C
	CLASS_PAINTER,           // 0x6D
	CLASS_TOURIST,           // 0x6E
};
